...
type CoorKey = `(${number},${number})`;
type Dir = "N" | "E" | "S" | "W";
type CoorDirKey = `(${number},${number},${Dir})`;

function toCoorKey(coor: Coor): CoorKey {
  return `(${coor[0]},${coor[1]})`;
}

function toCoor(coorKey: CoorKey): Coor {
  const data = coorKey.slice(1, coorKey.length - 1).split(",").map(
    Number.parseInt,
  );
